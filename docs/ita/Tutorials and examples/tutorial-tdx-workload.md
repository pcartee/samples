---
title: Trust Authority TDX Secure Key Release Workload Tutorial
description: Tutorial for deploying a TDX confidential workload that uses Trust Authority for passport attestation and secure key release.
author: pcartee
topic-type: tutorial
date: 12/19/2024
...
## Trust Authority TDX Secure Key Release Workload

The sample confidential computing workload for Trust Domain Extensions (TDX) demonstrates secure key release using the passport attestation model. The sample is provided in two Docker containers: a relying party and an attesting workload. Trust Authority is the verifier. The open-source code for this sample is available in the [trustauthority-samples][tdx-ml-workload] repository.

## Secure key release workflow

In this sample, the secure key release workflow operates in passport attestation mode. The TDX workload (CW) is the attester, KBS is the relying party, and Trust Authority is the verifier. For more information about key release and passport attestation models, see [Attestation Patterns](../Concepts/concept-patterns.md).

This sample shows how to implement two important use cases: secure key release with passport attestation and exchanging a public key, binary data, or JSON data up to 1 MiB between the relying party and the attester. The sample workload is a machine learning (ML) model distributed as an encrypted blob. A key is needed to decrypt the blob and enable execution of the model.
...
1. The attesting workload obtains a quote from the TDX TD by using the [Trust Authority CLI for TDX](../Integration/integrate-go-tdx-cli.md) ("TDX CLI"), which is also used to request an attestation [token](../Integration/integrate-go-tdx-cli.md#token) from Trust Authority.
      1. The workload service creates an RSA key pair. The private key is retained by the service, and the public key is base64-encoded and then copied to the `--user-data` parameter in the `token` command. The user data is output in the **attester_held_data** claim in the attestation token. The public key is used by KBS to encrypt (encapsulate) the key request response body.
1. Trust Authority evaluates the quote and returns an attestation token.
1. The workload forwards the attestation token to KBS in a REST API request for the decryption key needed to decrypt the ML model.
1. The KBS evaluates the request and applies its key release policy. A KBS key release policy compares values in the attestation token to reference values stored in the key release policy. If the key release policy evaluation is successful (that is, all compared values match), the KBS contacts the KMS to obtain the decryption key. The KMS also applies its own key release policy, which is independent of the KBS policy. If the KMS policy allows, the KMS returns the requested key or secret to the KBS. The KBS uses the public key passed in the attestation token to wrap the KMS-supplied decryption key before sending it back to the workload.
...
1. The workload service decrypts the KBS response by using the RSA private key from step 1.1. Then the service extracts the SWK from the response and uses it to decrypt the KMS-supplied key, which can then be used to decrypt the ML model. The model is now ready for use.
...
1. A machine learning model that represents a confidential computing application. The model is assumed to contain confidential data or methodology; therefore, it is encrypted. Before the TDX client can decrypt the model for use, it needs to obtain a decryption key from KBS.
...
- The service package manages the following:
  ...
  - Executes the CW's main function to decrypt and start the workload.
...
In this sample, [Trust Authority Key Broker Service][kbs-readme] (KBS) is the relying party. KBS is, as the name suggests, a _broker_ between the workload and the key management system (KMS). KBS doesn't store or manage keys and secrets; it relies on the KMS to do that.
...
:::important
PyKMIP isn't a secure, production-quality KMS. Don't use PyKMIP to store actual secrets. KBS supports PyKMIP for development and HashiCorp Vault (free or paid) for production use.
:::
...
- **workload.env** to supply the workload with KBS and Trust Authority connection information. This file also contains secrets.
...
:::caution
KBS.env contains secrets including an administrator password, KMS credentials, and a Trust Authority attestation API key. To help secure your system, delete the KBS.env file immediately after the initial configuration of the KBS container. However, you don't need to delete the KBS.env file for the sample app if you're using it only as a learning tool.
:::

When the relying-party container runs, a startup script configures KBS to use the `ADMIN_USERNAME` and `ADMIN_PASSWORD` provided in the **KBS.env** file. You'll also need to provide an attestation API key for `TRUSTAUTHORITY_API_KEY`, which you can get from the Trust Authority [portal][ita-portal].
...
1. Download the `sample-workload/execute_workload_flow.sh` script to execute the secure key release workflow.

    ```bash
    wget https://raw.githubusercontent.com/[redacted]/execute_workload_flow.sh
    ```

1. Run the following command to execute the workflow script.
